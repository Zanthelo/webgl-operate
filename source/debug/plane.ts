...
export class Plane extends Geometry {
  ...
  initialize(aVertex: GLuint): boolean {
    ...
    this._buffers[0].data(Plane.VERTICES, gl.STATIC_DRAW);
    this._buffers[1].data(Plane.INDICES, gl.STATIC_DRAW);
  ...
  draw(): void {
    const gl = this.context.gl;
    gl.drawElements(gl.TRIANGLE_STRIP, Plane.INDICES.length, gl.UNSIGNED_BYTE, 0);
  }
